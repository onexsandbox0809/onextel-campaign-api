...
export default async function handler(req, res) {
  ...
  const keyword2 = requiredString(body.keyword2, 200);
  ...
  if (!keyword2) errors.push('keyword2 is required.');
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{ mobile_number, keyword2, reason, other, response_date, response_time }])
      .select('id, mobile_number, keyword2, reason, other, response_date, response_time')
      .single();

    if (error) {
      console.error('keyword2 insert error:', error);
      return res.status(500).json({ success: false, error: 'Failed to save keyword2 response.' });
    }

    return res.status(201).json({ success: true, data });
  } catch (err) {
    console.error('keyword2 handler error:', err);
